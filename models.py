"""
MAE model components — designed for clarity when teaching.

Flow:
  1. PatchEmbedding: image → patch tokens
  2. Encoder: only visible patches → latent vectors (no mask tokens)
  3. Decoder: latent + mask tokens → predictions for all patches
  4. Loss on masked patches only (reconstruct pixel values)
"""
...
# =============================================================================
# 1. Patch embedding: turn image into a sequence of patch tokens
# =============================================================================

class PatchEmbedding(nn.Module):
    """
    Split image into patches and project each patch to a vector.
    Input: (B, 3, 224, 224)  →  Output: (B, 196, 768) for encoder dimension.
    """
    ...
    def __init__(self, embed_dim):
        ...
        # Linear projection: (patch_pixels * channels) -> embed_dim
        # Each patch: 16*16*3 = 768 values → one vector of size embed_dim
...
# =============================================================================
# 2. Transformer layers: we use PyTorch's built-in TransformerEncoderLayer
#    (LayerNorm → Multi-Head Attention → residual → LayerNorm → MLP → residual)
# =============================================================================


# =============================================================================
# 3. Encoder: ViT-Base — only visible patches, no mask tokens
# =============================================================================

class Encoder(nn.Module):
    """
    Vision Transformer encoder (ViT-Base).
    - Input: visible patch tokens only (25% of patches).
    - Adds positional embeddings (for visible positions).
    - Output: latent vectors for those visible patches only.
    """
    ...
    def __init__(self):
        ...
        # Stack of transformer layers (PyTorch built-in)
...
# =============================================================================
# 4. Decoder: ViT-Small — reconstructs all patches from latent + mask tokens
# =============================================================================

class Decoder(nn.Module):
    """
    Lightweight decoder (ViT-Small).
    - Input: encoder latent (visible) + learnable mask tokens (masked positions).
    - All positions get positional embeddings so the decoder knows where each token is.
    - Output: per-patch vectors that we project to pixel space for reconstruction.
    """
...
# =============================================================================
# 5. MAE: full model with random masking
# =============================================================================

class MAE(nn.Module):
    """
    Masked Autoencoder:
    1. Randomly mask 75% of patches (encoder sees 25%).
    2. Encoder: embed image, keep only visible tokens, add pos embed, run ViT-Base.
    3. Decoder: add mask tokens for masked positions, run ViT-Small, predict pixels.
    4. Loss: MSE on masked patch pixels only.
    """
    ...
    def _random_mask(self, B, device):
        ...
        visible_indices = ids[:, :self.num_visible]      # (B, 49)
        mask_indices = ids[:, self.num_visible:]         # (B, 147)
        return visible_indices, mask_indices

    def forward(self, x, visible_indices=None, mask_indices=None):
        """
        x: (B, 3, 224, 224)
        If visible_indices / mask_indices not given, random mask is used (training).
        """
        B = x.shape[0]
        device = x.device
        ...
        # Encoder: only visible patches → latent
        latent = self.encoder(x, visible_indices)  # (B, 49, 768)

        # Decoder: latent + mask tokens → predict masked patch pixels
        pred_masked = self.decoder(latent, visible_indices, mask_indices)  # (B, 147, 768)
        return pred_masked, visible_indices, mask_indices


# =============================================================================
# Helper: get patch pixels from image (for loss target)
# =============================================================================

def image_to_patch_pixels(x):
    """
    Turn image into patch pixels (no projection). Same grid as PatchEmbedding.
...
# =============================================================================
# Loss: MSE on masked patches (in pixel space)
# =============================================================================
...
def mae_loss(model_out, target_patches, mask_indices):
    ...
    Patch normalization (per-patch mean/var) so the model learns structure rather than
    average brightness, improving reconstruction quality (MAE paper).
    """
